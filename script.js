...
gameOverSound.volume = 0.85;// Global mouse position for particle reactivity
let mousePos = { x: 0, y: 0 };// Score management
let score = 0;
let scoreEl = null;// Health management for bar
let health = parseFloat(sessionStorage.getItem('health')) || 100;
let healthBar = null;
let healthProgress = null;
let gameOverShown = false;// Function to wait for audio to finish playing
...
function explode(mx, my, hexColor) {
  ...
  if (percent <= 0) {
    if (!gameOverShown) {
      ...
      healthBar.appendChild(gameOverText);  const onSoundEnd = async () => {
...
return;  }
  ...
  healthProgress = document.getElementById("health-progress");
  if (!healthBar || !healthProgress) return;  // Initial update
  updateHealthBar();
  ...
  else color = '#00FFFF'; // Cyan  const canvas = document.createElement('canvas');
  const size = 200;
  ...
  ctx.textShadow = 0 0 3px ${color};  document.body.appendChild(canvas);  // Delay particle effect by 300ms
  setTimeout(() => {
...
function generateNav() {
  let navHTML =     <nav class="navbar navbar-expand-lg navbar-dark bg-dark" style="position: fixed !important; top: 0; left: 0; right: 0; z-index: 1000 !important;">       <div class="container">         <a class="navbar-brand" href="index.html">           <img src="img/BEARDsmall.png" alt="Lance Woolie" style="height: 20px;"> Lance Woolie         </a>         <div class="score-header mx-auto d-flex justify-content-center align-items-center gap-3">           <div class="health-display">             <div class="health-bar">               <div id="health-bar">                 <div id="health-progress"></div>               </div>             </div>           </div>           <div class="score-display">             <span class="score-label">Score</span> <span id="score-value">000000</span> <!-- Removed "Your" -->           </div>         </div>         <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav">           <span class="navbar-toggler-icon"></span>         </button>         <div class="collapse navbar-collapse" id="navbarNav">           <ul class="navbar-nav ms-auto">             <li class="nav-item"><a class="nav-link" href="index.html">Home</a></li>             <li class="nav-item dropdown">               <a class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">Music</a>               <ul class="dropdown-menu horizontal-dropdown">                 <li><a class="dropdown-item d-flex align-items-center" href="https://orcd.co/a4glqme" target="_blank">                   <img src="img/Ubermenu.jpg" alt="Uber" class="dropdown-img me-2" style="width: 150px; height: 150px; object-fit: cover;">                   Uber                 </a></li>                 <li><a class="dropdown-item d-flex align-items-center" href="https://orcd.co/lancewoolietoodrunk" target="_blank">                   <img src="img/TooDrunkmenu.jpg" alt="Too Drunk" class="dropdown-img me-2" style="width: 150px; height: 150px; object-fit: cover;">                   Too Drunk                 </a></li>                 <li><a class="dropdown-item d-flex align-items-center" href="https://orcd.co/lancewoolieworstenemy" target="_blank">                   <img src="img/WorstEnemymenu.jpg" alt="Worst Enemy" class="dropdown-img me-2" style="width: 150px; height: 150px; object-fit: cover;">                   Worst Enemy                 </a></li>                 <li><a class="dropdown-item d-flex align-items-center" href="music.html">                   <img src="img/Fullcatalogmenu.jpg" alt="Full Catalog" class="dropdown-img me-2" style="width: 150px; height: 150px; object-fit: cover;">                   Full Catalog                 </a></li>               </ul>             </li>             <li class="nav-item"><a class="nav-link" href="events.html">Events</a></li>             <li class="nav-item"><a class="nav-link" href="origins.html">Origins</a></li>             <li class="nav-item"><a class="nav-link" href="merch.html">Merch</a></li>             <li class="nav-item"><a class="nav-link" href="contact.html">Contact</a></li>           </ul>         </div>       </div>     </nav>  ;  // Auto-Highlight Active Page (Enhanced for Dropdowns)
  ...
  navHTML = tempDiv.innerHTML;  const placeholder = document.getElementById('nav-placeholder');
  if (placeholder) {
...
function generateFooter() {
  const footerHTML =     <footer class="footer bg-dark text-light py-1" style="position: fixed !important; bottom: 0; left: 0; right: 0; z-index: 1001 !important; border-top: 1px solid #0074D9;">       <div class="container">         <!-- Bottom Row: Contact, Copyright, Icons -->         <div class="row align-items-center">           <div class="col-md-4 d-flex align-items-center">             <a href="contact.html" class="text-light me-3">Contact</a>             <div class="weapon-select">               <select id="weapon-dropdown">                 <option selected>Laser sights (lv 1)</option>                 <option disabled>2x Barrel Shogun (lv 2)</option>                 <option disabled>Bazooka (lv 3)</option>               </select>             </div>           </div>           <div class="col-md-4 text-center">             <div class="cowboy-hat-icon mb-1" id="cowboy-hat"></div>             <p class="mb-0 small">&copy; 2025 Lance Woolie. All rights reserved.</p>           </div>           <div class="col-md-4 text-end">             <a href="https://x.com/LanceWoolie" target="_blank" class="me-3" title="X (Twitter)" style="display: inline-block; width: 24px; height: 24px; margin-right: 8px;">               <img src="https://cdn.simpleicons.org/x/ffffff/24.svg" alt="X" style="width: 24px; height: 24px;">             </a>             <a href="https://www.facebook.com/lancewooliemusic/" target="_blank" class="me-3" title="Facebook" style="display: inline-block; width: 24px; height: 24px; margin-right: 8px;">               <img src="https://cdn.simpleicons.org/facebook/ffffff/24.svg" alt="Facebook" style="width: 24px; height: 24px;">             </a>             <a href="https://www.tiktok.com/@lancewoolie" target="_blank" class="me-3" title="TikTok" style="display: inline-block; width: 24px; height: 24px; margin-right: 8px;">               <img src="https://cdn.simpleicons.org/tiktok/ffffff/24.svg" alt="TikTok" style="width: 24px; height: 24px;">             </a>             <a href="https://www.youtube.com/channel/UC9NUm7_BejCwctJ9u6dps7g" target="_blank" class="me-3" title="YouTube" style="display: inline-block; width: 24px; height: 24px; margin-right: 8px;">               <img src="https://cdn.simpleicons.org/youtube/ffffff/24.svg" alt="YouTube" style="width: 24px; height: 24px;">             </a>             <a href="https://www.instagram.com/lancewoolie/" target="_blank" title="Instagram" style="display: inline-block; width: 24px; height: 24px;">               <img src="https://cdn.simpleicons.org/instagram/ffffff/24.svg" alt="Instagram" style="width: 24px; height: 24px;">             </a>           </div>         </div>       </div>       <!-- Subscribe Raccoon Behind Footer -->       <img id="subscribe-raccoon" class="floating-subscribe" src="img/racoon.png" alt="Subscribe Raccoon">     </footer>  ;  let placeholder = document.getElementById('footer-placeholder');
  if (!placeholder) {
  ...
  placeholder.innerHTML = footerHTML;  // Ensure footer is always on top
  ...
  if (cowboyHat) {
    ...
    const surprises = [
      ...
      () => { cowboyHat.textContent = ''; setTimeout(() => cowboyHat.textContent = '', 1000); },
      () => { window.scrollTo({ top: 0, behavior: 'smooth' }); cowboyHat.textContent = ''; setTimeout(() => cowboyHat.textContent = '', 1000); }
    ];
    cowboyHat.addEventListener('click', () => surprisesMath.floor(Math.random() * surprises.length) );
  }
}// Load Nav & Footer on DOM Ready
document.addEventListener('DOMContentLoaded', () => {
  // Initialize score
  score = parseInt(sessionStorage.getItem('score')) || 0;  generateNav();
  generateFooter();  // Init score display after nav generation
  ...
  setTimeout(initHealthBar, 100); // Delay for render  const currentPage = window.location.pathname.split('/').pop().replace('.html', '') || 'index';  // Add Neon Cursor Toy effect to all pages except index
  if (currentPage !== 'index') {
    // Add Montserrat font
    const fontLink = document.createElement('link');
    fontLink.href = 'https://fonts.googleapis.com/css2?family=Montserrat:wght@500;700&display=swap';
    fontLink.rel = 'stylesheet';
    document.head.appendChild(fontLink);// Add neon cursor styles
...
`;
document.head.appendChild(neonScript);  }  // Global sub-dot click handlers (works on index, empty on other pages)
  document.querySelectorAll('.sub-dot').forEach(sub => {
    sub.addEventListener('click', async (e) => {
      ...
      const mainDot = container ? container.querySelector('.main-dot') : null;  const clickedSubDots = JSON.parse(sessionStorage.getItem('clickedSubDots') || '[]');
  ...
  if (currentPage === 'index') {
    window.addEventListener('load', () => {
      ...
      const dots = document.querySelectorAll('.main-dot');  // Shake the body
